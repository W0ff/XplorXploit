...
const App: React.FC = () => {
  ...
  const [presets, setPresets] = useState<Record<string, LogicBlock[]>>(() => {
    ...
    return Object.fromEntries(Object.entries(DEFAULT_PRESETS).map(([k, v]) => [k, v.blocks]));
  });
  ...
  const setStrategy = useCallback((updater: React.SetStateAction<LogicBlock[]>) => {
    setPresets(prev => {
      const newStrategy = typeof updater === 'function' ? updater(prev[activePresetKey]) : updater;
  ...
  const resetPresets = () => {
    const freshPresets = Object.fromEntries(Object.entries(DEFAULT_PRESETS).map(([k, v]) => [k, v.blocks]));
